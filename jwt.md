...
??? Это можно упомянуть, но не особо важная информация.
Иногда можно встретить еще упоминания JWS или JWE.
То есть JWT = общий формат токена, JWS = подписанный JWT, а JWE = шифрованный JWT (шифрование payload).
На практике почти всегда встречается JWS (т.е. "обычный JWT"), а JWE используется редко.
Подпись и шифрование решают разные задачи: подпись = доверие, шифрование = секретность.
